...
import { toast } from '@/components/ui/use-toast'
import { EditorBtns } from '@/lib/constants'
import { getFunnel, getUserDetails } from '@/lib/queries'
import { getStripe } from '@/lib/stripe/stripe-client'
import { EditorElement, useEditor } from '@/providers/editor/editor-provider'
import {
  EmbeddedCheckout,
...
const Checkout = (props: Props) => {
  const { dispatch, state, userId, funnelId, pageDetails } = useEditor()
  const router = useRouter()
  const [clientSecret, setClientSecret] = useState('')
  const [livePrices, setLivePrices] = useState([])
  const [userConnectAccId, setUserConnectAccId] = useState('')
  const options = useMemo(() => ({ clientSecret }), [clientSecret])
  const styles = props.element.styles
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      if (userDetails) {
        if (!userDetails.connectAccountId) return
        setUserConnectAccId(userDetails.connectAccountId)
      }
    }
    fetchData()
  }, [userId])
  ...
  useEffect(() => {
    if (livePrices.length && userId && userConnectAccId) {
  ...
  }, [livePrices, userId, userConnectAccId])
  ...
  return (
    ...
    >
      ...
      <div className="border-none transition-all w-full">
        <div className="flex flex-col gap-4 w-full">
          {options.clientSecret && userConnectAccId && (
            <div className="text-white">
              <EmbeddedCheckoutProvider
                stripe={getStripe(userConnectAccId)}
                options={options}
              >
                <EmbeddedCheckout />
              </EmbeddedCheckoutProvider>

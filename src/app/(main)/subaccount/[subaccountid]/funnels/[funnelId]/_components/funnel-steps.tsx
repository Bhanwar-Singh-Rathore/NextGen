...
import { toast } from "@/components/ui/use-toast";
import { upsertFunnelPage } from "@/lib/queries";
import { FunnelsForUser } from "@/lib/types";
import { useModal } from "@/providers/modal-provider";
import { FunnelPage } from "@prisma/client";
import { Check, ExternalLink, LucideEdit } from "lucide-react";
import React, { useState } from "react";
...
type Props = {
  funnel: FunnelsForUser;
  userId: string;
  pages: FunnelPage[];
  funnelId: string;
};

const FunnelSteps = ({ funnel, funnelId, pages, userId }: Props) => {
  const [clickedPage, setClickedPage] = useState<FunnelPage | undefined>(
    pages[0]
  );
  const { setOpen } = useModal();
            ...
            <Card className="h-full flex justify-between flex-col">
              <CardHeader>
                ...
                <CardDescription className="flex flex-col gap-4">
                  <div className="border-2 rounded-lg sm:w-80 w-full overflow-clip">
                    {/* <Link
                      href={`/subaccount/${userId}/funnels/${funnelId}/editor/${clickedPage?.id}`}
                      className="relative group"
                    > */}
                    <Link
                      href={`/subaccount/${
                        userId || params.userId
                      }/funnels/${funnelId}/editor/${clickedPage?.id}`}
                      className="relative group"
                    >
                      <div className="cursor-pointer group-hover:opacity-30 w-full">
                        <FunnelPagePlaceholder />

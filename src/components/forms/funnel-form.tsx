...
interface CreateFunnelProps {
  defaultData?: any; // Adjust according to your data structure
  userId: string; // Pass userId as a prop
}

const FunnelForm: React.FC<CreateFunnelProps> = ({ defaultData, userId }) => {
  const { setClose } = useModal();
  const router = useRouter();
  const form = useForm<z.infer<typeof CreateFunnelFormSchema>>({
    mode: 'onChange',
  ...
  const onSubmit = async (values: z.infer<typeof CreateFunnelFormSchema>) => {
    ...
    // Validate values
    const result = CreateFunnelFormSchema.safeParse(values);
    if (!result.success) {
      console.error('Validation Errors:', result.error.flatten().fieldErrors); // Log validation errors
      return;
    }

    if (!userId) {
      console.error('No userId provided'); // Log the error if userId is not provided
      return;
    }

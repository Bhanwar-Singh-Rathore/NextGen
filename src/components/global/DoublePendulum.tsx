...
const DoublePendulum = () => {
  useEffect(() => {
    ...
    Example.doublePendulum = function() {
      ...
      // create renderer with background color from CSS
      var render = Render.create({
        element: document.getElementById('pendulum-container'), // Render in this div
        engine: engine,
        options: {
          width: 400,
          height: 300,
          wireframes: false,
          background: getComputedStyle(document.body).getPropertyValue('--bg-color') // Dynamic background color
        }
      });
    ...
    // Call the double pendulum function
    const pendulumSimulation = Example.doublePendulum();

    // Cleanup function
    return () => {
      pendulumSimulation.stop();
    };
  }, []);
